...
@Component({
  selector: 'app-site-image-edit-admin',
  templateUrl: './site-image-edit-admin.component.html'
})
export class SiteImageEditAdminComponent extends AbstractEditController<SiteImageDto, SiteImageCriteria, SiteImageAdminService>   implements OnInit {
   ...
   public async openCreateSite(site: string) {
        const isPermistted = await this.roleService.isPermitted('Site', 'edit');
        if (isPermistted) {
             this.site = new SiteDto();
